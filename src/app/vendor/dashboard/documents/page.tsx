              ...
              if (!authLetter) {
                return (
                  ...
                  >
                    ...
                    <h3 className="text-lg font-black text-secondary mb-1">{t('onboarding.vendorAgreement', 'Vendor Agreement')}</h3>
                    <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-2 flex items-center gap-1.5">
                      {t('onboarding.awaitingGeneration', 'Awaiting Generation')}
                    </p>
                    <p className="text-xs text-gray-400 font-bold leading-relaxed mb-4">
                      {t('onboarding.vendorAgreementDesc1', 'Official digital contract between the partner and the platform. This will be generated by the Admin after initial KYC review.')}
                    </p>
                    <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100 flex items-start gap-3">
                      <AlertCircle size={16} className="text-amber-500 shrink-0 mt-0.5" />
                      <div>
                        <p className="text-[10px] text-amber-700 font-black uppercase tracking-widest mb-1">
                          {t('onboarding.generationPending', 'Generation Pending')}
                        </p>
                        <p className="text-[10px] text-amber-600 font-bold leading-relaxed">
                          {t('onboarding.agreementPendingDesc', 'Your agreement is currently awaiting generation by the administrator. Once it is generated, you will be able to review, download, and upload the signed copy here.')}
                        </p>
                      </div>
                    </div>
                  </motion.div>
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[10px] text-primary font-black uppercase tracking-widest mb-2 flex items-center gap-1.5">
                    {t('onboarding.agreementId', 'Agreement ID')}: {authLetter.agreementId || 'N/A'}
                  </p>
                  <p className="text-xs text-gray-400 font-bold leading-relaxed mb-4">
                    {t('onboarding.vendorAgreementDesc2', 'Official digital contract between the partner and the platform. Please download the document, sign it physically or digitally, and upload the signed copy.')}
                  </p>
                    ...
                    <div className="p-4 bg-white/70 rounded-2xl border border-gray-100 flex items-center justify-between gap-3 shadow-sm">
                      <div className="min-w-0 flex-1">
                        <span className="text-[9px] font-black uppercase tracking-widest px-2.5 py-0.5 rounded-full bg-blue-100 text-blue-600">
                          {t('onboarding.readyForDownload', 'Ready for download')}
                        </span>
                        <p className="text-sm font-black text-secondary mt-2 truncate">{t('onboarding.vendorAgreementGeneratedCopy', 'Vendor Agreement (Generated Copy)')}</p>
                        {authLetter.createdAt && (
                          <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">
                            {t('onboarding.generatedAt', 'Generated {{date}}', { date: new Date(authLetter.createdAt).toLocaleDateString() })}
                          </p>
                        )}
                      </div>
                      <a
                      ...
                      >
                        <Download size={14} /> {t('onboarding.download', 'Download')}
                      </a>
                    </div>
                      ...
                      <div className="flex flex-col gap-3 bg-white/70 p-3 rounded-2xl border border-gray-100">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 mb-2">
                              <span className="text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full bg-green-100 text-green-600">
                                {t('onboarding.signedCopyUploaded', 'Signed Copy Uploaded')}
                              </span>
                              <span className={`text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full ${
                                ...
                                {isLocked || status === 'approved' ? t('onboarding.approvedLocked', 'Approved & Locked') :
                                 status === 'rejected' ? t('status.rejected', 'Rejected') :
                                 status === 'reupload_required' ? t('onboarding.reuploadRequired', 'Re-upload Required') : t('status.under_review', 'Under Review')}
                              </span>
                            </div>
                        ...
                        <div className="flex items-center justify-between gap-3 pt-2 border-t border-gray-100">
                          <a href={getDocumentViewUrl(authLetter.uploadedDocumentUrl)} target="_blank" rel="noopener noreferrer" className="text-[10px] font-black text-primary uppercase tracking-widest hover:underline flex items-center gap-2">
                            {t('onboarding.previewOpenSignedPdf', 'Preview / Open Signed PDF')} <ChevronRight size={14} />
                          </a>
                        </div>
                      </div>
                    ) : null}
                    ...
                    {!isLocked && status !== 'approved' && (
                      <label className={`w-full py-3 rounded-2xl flex items-center justify-center gap-3 font-black text-xs uppercase tracking-widest cursor-pointer transition-all ${
                        ...
                        {uploadingDocId === authLetter._id ? (
                          ...
                          <><Upload size={16} /> {hasUploaded ? t('onboarding.replaceSignedDocument', 'Replace Signed Document') : t('onboarding.uploadSignedCopy', 'Upload Signed Copy')}</>
                        )}
                        <input 
                          type="file" 
                          className="hidden" 
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-soft">
              ...
              <div className="flex flex-col gap-3">
                ...
                {[
                  { id: 'auth_letter', titleKey: 'onboarding.vendorAgreement', defaultTitle: 'Vendor Agreement', icon: FileText },
                  { id: 'vendor_code_cert', titleKey: 'onboarding.vendorCodeCertificate', defaultTitle: 'Vendor Code Certificate', icon: ShieldCheck }
