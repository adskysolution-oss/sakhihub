      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        <div>
          <h2 style={{ fontSize: '2rem', fontWeight: '900', color: 'var(--secondary)' }}>
            {t('employeeReports.title', 'Daily Activity Reports')}
          </h2>
          <p style={{ color: 'var(--text-muted)' }}>
            {t('employeeReports.subtitle', 'Log your daily work and track your field performance.')}
          </p>
        </div>
        <button onClick={() => setShowAdd(true)} className="btn-primary" style={{ gap: '10px' }}>
          <Plus size={20} /> {t('employeeReports.submitBtn', 'Submit New Report')}
        </button>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
        {loading ? (
          <p>{t('employeeReports.loading', 'Loading reports...')}</p>
        ) : reports.map((report) => (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 1fr', alignItems: 'center', gap: '30px', minWidth: '700px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px' }}>
                 <div style={{ textAlign: 'center' }}>
                   <p style={{ margin: 0, fontSize: '0.8rem', color: '#888', fontWeight: '700' }}>{t('employeeReports.groups', 'Groups')}</p>
                   <p style={{ margin: '5px 0 0', fontSize: '1.2rem', fontWeight: '900' }}>{report.groupsCreated}</p>
                 </div>
                 <div style={{ textAlign: 'center' }}>
                   <p style={{ margin: 0, fontSize: '0.8rem', color: '#888', fontWeight: '700' }}>{t('employeeReports.members', 'Members')}</p>
                   <p style={{ margin: '5px 0 0', fontSize: '1.2rem', fontWeight: '900' }}>{report.membersAdded}</p>
                 </div>
                 <div style={{ textAlign: 'center' }}>
                   <p style={{ margin: 0, fontSize: '0.8rem', color: '#888', fontWeight: '700' }}>{t('employeeReports.collection', 'Collection')}</p>
                   <p style={{ margin: '5px 0 0', fontSize: '1.2rem', fontWeight: '900' }}>₹{report.membershipCollected}</p>
                 </div>
                 <div style={{ textAlign: 'center' }}>
                   <p style={{ margin: 0, fontSize: '0.8rem', color: '#888', fontWeight: '700' }}>{t('employeeReports.inquiries', 'Inquiries')}</p>
                   <p style={{ margin: '5px 0 0', fontSize: '1.2rem', fontWeight: '900' }}>{report.padsInquiry}</p>
                 </div>
              </div>

              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '15px' }}>
                 {report.status === 'verified' && <span style={{ color: '#10b981', background: '#f0fdf4', padding: '8px 15px', borderRadius: '10px', fontSize: '0.85rem', fontWeight: '800', display: 'flex', alignItems: 'center', gap: '5px' }}><CheckCircle2 size={16} /> {t('employeeReports.verified', 'Verified')}</span>}
                 <button style={{ color: '#666', background: '#f8f9fa', padding: '10px 20px', borderRadius: '12px', border: 'none', fontWeight: '700', cursor: 'pointer' }}>{t('employeeReports.viewDetails', 'View Details')}</button>
              </div>
            </div>
          </div>
        ))}
        {reports.length === 0 && !loading && (
          <div style={{ textAlign: 'center', padding: '80px 20px', background: 'white', borderRadius: '30px' }}>
             ...
             <h3 style={{ color: '#999' }}>{t('employeeReports.noReports', 'No reports submitted yet.')}</h3>
          </div>
        )}
      </div>
    </DashboardLayout>
