...
export default function GroupDetailsPage() {
  ...
  if (loading) return <DashboardLayout><p>{t('employeeGroups.loadingDetails', 'Loading group details...')}</p></DashboardLayout>;
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '20px' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: '15px', color: 'var(--text-muted)', marginTop: '10px', flexWrap: 'wrap' }}>
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><Users size={16} /> {members.length} {t('employeeGroups.membersRegistered', 'Members Registered')}</span>
              <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><ShieldCheck size={16} /> {t('employeeGroups.leader', 'Leader')}: {group.leaderName}</span>
            </div>
          </div>
          <button onClick={() => setShowAddMember(true)} className="btn-primary" style={{ gap: '10px', padding: '15px 30px' }}>
            <UserPlus size={20} /> {t('employeeGroups.addNewMember', 'Add New Member')}
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 350px', gap: '30px', alignItems: 'start' }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ textAlign: 'left', borderBottom: '2px solid #f8f9fa' }}>
                    ...
                    <th style={{ padding: '15px', color: '#999', fontSize: '0.8rem', fontWeight: '800', textTransform: 'uppercase' }}>{t('employeeGroups.actionCol', 'Action')}</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={member._id} style={{ borderBottom: '1px solid #f8f9fa' }}>
                      <td style={{ padding: '15px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                          ...
                          <div>
                            ...
                            <p style={{ fontSize: '0.75rem', color: '#999', margin: 0 }}>{t('employeeGroups.joined', 'Joined')} {new Date(member.createdAt).toLocaleDateString()}</p>
                          </div>
                        </div>
                      </td>
                      <td style={{ padding: '15px' }}>
                      ...
                      <td style={{ padding: '15px' }}>
                        <button style={{ background: 'none', border: 'none', color: 'var(--primary)', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}>
                          {t('employeeGroups.details', 'Details')} <ChevronRight size={16} />
                        </button>
                      </td>
                    </tr>
                  ))}
                  {filteredMembers.length === 0 && (
                    <tr>
                      <td colSpan={4} style={{ textAlign: 'center', padding: '40px', color: '#999' }}>{t('employeeGroups.noMembers', 'No members found in this group.')}</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        <div>
          <div className="glass-card" style={{ background: 'white', padding: '25px', borderRadius: '25px', marginBottom: '20px' }}>
            <h4 style={{ fontWeight: '900', marginBottom: '20px', fontSize: '1.1rem' }}>{t('employeeGroups.overview', 'Group Overview')}</h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              <div style={{ padding: '15px', background: '#f8f9fa', borderRadius: '15px' }}>
                <p style={{ margin: 0, fontSize: '0.75rem', color: '#999', fontWeight: '800' }}>{t('employeeGroups.villageUnit', 'VILLAGE UNIT')}</p>
                <p style={{ margin: '5px 0 0', fontWeight: '700' }}>{group.village}, {group.block}</p>
              </div>
              <div style={{ padding: '15px', background: '#f8f9fa', borderRadius: '15px' }}>
                <p style={{ margin: 0, fontSize: '0.75rem', color: '#999', fontWeight: '800' }}>{t('employeeGroups.groupLeader', 'GROUP LEADER')}</p>
              ...
              <div style={{ padding: '15px', background: '#f8f9fa', borderRadius: '15px' }}>
                <p style={{ margin: 0, fontSize: '0.75rem', color: '#999', fontWeight: '800' }}>{t('employeeGroups.nextMeetingLabel', 'NEXT MEETING')}</p>
              ...
              {group.campaignId && (
                <div style={{ padding: '15px', background: '#fffcf0', borderRadius: '15px', border: '1px solid #fef3c7' }}>
                  <p style={{ margin: 0, fontSize: '0.75rem', color: '#d97706', fontWeight: '800' }}>{t('employeeGroups.activeCampaign', 'ACTIVE CAMPAIGN')}</p>
          ...
          <div style={{ background: 'var(--grad-primary)', padding: '25px', borderRadius: '25px', color: 'white' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '15px' }}>
              <CheckCircle size={24} />
              <h4 style={{ margin: 0, fontWeight: '900' }}>{t('employeeGroups.activationStats', 'Activation Stats')}</h4>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
              <span>{t('employeeGroups.paidMembers', 'Paid Members')}:</span>
              <span style={{ fontWeight: '800' }}>{members.filter(m => m.membershipStatus === 'paid').length} / {members.length}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span>{t('employeeGroups.activationRate', 'Activation Rate')}:</span>
