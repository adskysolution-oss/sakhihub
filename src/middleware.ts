...
export async function middleware(request: NextRequest) {
  ...
  const isPublicPage = !isAdminPage && !isEmployeePage && !isMemberPage && !isAuthPage;
  ...
  if (isAdminPage || isEmployeePage || isMemberPage) {
    if (!token) {
    ...
    try {
      ...
      if (isEmployeePage && payload.role !== 'employee' && payload.role !== 'super_admin') {
        return NextResponse.redirect(new URL('/unauthorized', request.url));
      }
      if (isMemberPage && payload.role !== 'member' && payload.role !== 'employee' && payload.role !== 'super_admin') {
        return NextResponse.redirect(new URL('/unauthorized', request.url));
      }
    } catch (e) {
