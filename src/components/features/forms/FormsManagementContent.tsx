...
export default function FormsManagementContent() {
    ...
    <div className="flex flex-col gap-8">
      ...
      {loading ? (
        ...
        <div className="bg-white p-12 rounded-[24px] shadow-sm border border-gray-100 text-center">
          ...
          {canManage && (
            <Link href={`${basePath}/builder`} className="text-primary font-bold hover:underline">
          ...
          {forms.map((form) => (
            <div key={form._id} className="bg-white rounded-[24px] p-6 shadow-sm border border-gray-100 relative group flex flex-col h-full">
              ...
              <div className="grid grid-cols-2 gap-2 pt-4 border-t border-gray-100 mt-auto">
                <Link href={`${basePath}/${form._id}/responses`} className="bg-blue-50 hover:bg-blue-100 text-blue-600 py-2 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-colors">
                  <List size={14} /> Responses
                </Link>
                {canManage ? (
                  <Link href={`${basePath}/builder?id=${form._id}`} className="bg-gray-50 hover:bg-gray-100 text-gray-700 py-2 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-colors">
                    <Edit size={14} /> Edit
                  </Link>
                ) : (
