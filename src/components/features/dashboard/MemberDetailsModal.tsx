        ...
        <section className="bg-gradient-to-br from-primary to-secondary-dark p-8 md:p-14 text-white relative">
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-10 text-center md:text-left mt-4 md:mt-0">
            ...
            <div className="flex-1">
              <h2 className="text-3xl md:text-5xl font-bold tracking-tight leading-tight mb-4">{member.name}</h2>
              <div className="flex flex-wrap justify-center md:justify-start gap-3">
                <span className="bg-white/20 backdrop-blur-md px-4 py-2 rounded-2xl text-[10px] md:text-xs font-semibold tracking-widest uppercase flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-16">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-6">
                  {member.interests?.map((interest: string, i: number) => (
                    <span key={i} className="bg-primary/5 text-primary px-5 py-2.5 rounded-2xl text-[11px] font-semibold uppercase tracking-widest border border-primary/10 shadow-sm">
                      {interest}
                    </span>
                  ))}
                </div>
            ...
            <div className="flex flex-col gap-10">
              ...
              <div>
                ...
                <div className={`mt-6 p-8 rounded-[32px] border-2 flex flex-col gap-6 ${member.membershipStatus === 'paid'
                    ? 'bg-green-50/50 border-green-100 text-green-700'
                    : 'bg-amber-50/50 border-amber-100 text-amber-700'
                  }`}>
                  <div className="flex justify-between items-start gap-4">
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-widest opacity-60 mb-2">Activation Status</p>
                      <h4 className="text-2xl font-bold leading-tight">
                        {member.membershipStatus === 'paid' ? 'Verified Member' : 'Pending Verification'}
                      </h4>
                    </div>
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${member.membershipStatus === 'paid' ? 'bg-green-100 text-green-600' : 'bg-amber-100 text-amber-600'
                      }`}>
...
function SectionTitle({ icon: Icon, title }: { icon: any, title: string }) {
  return (
    <div className="flex items-center gap-3 text-lg md:text-xl font-bold text-secondary uppercase tracking-tight">
...
function InfoItem({ label, value }: { label: string, value: string }) {
  return (
    <div className="flex flex-col gap-1.5 p-2 border-l-2 border-primary/10 pl-6">
      ...
      <p className="text-base md:text-lg font-bold text-secondary leading-tight">{value || 'N/A'}</p>
    </div>
  );
}
