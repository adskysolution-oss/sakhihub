        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div key="step1" {...fadeInUp} style={{ display: 'grid', gap: '20px' }}>
              <div onClick={() => setRole("member")} style={{ padding: '25px', borderRadius: '24px', border: '2px solid', borderColor: role === "member" ? 'var(--primary)' : '#f0f0f0', background: role === "member" ? '#FFF5F8' : 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '20px' }}>
                ...
                <div style={{ flex: 1 }}>
                  <h3 style={{ fontSize: '1.2rem', fontWeight: '800', color: 'var(--secondary)' }}>Join as Member</h3>
                  <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>Get community support & products.</p>
                </div>
              </div>
              <div onClick={() => setRole("employee")} style={{ padding: '25px', borderRadius: '24px', border: '2px solid', borderColor: role === "employee" ? 'var(--primary)' : '#f0f0f0', background: role === "employee" ? '#FFF5F8' : 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '20px' }}>
              ...
              <button type="button" disabled={!role} onClick={nextStep} className="btn-primary" style={{ marginTop: '20px', justifyContent: 'center', opacity: role ? 1 : 0.5 }}>Continue <ArrowRight size={20} /></button>
            </motion.div>
          )}
            ...
            <motion.div key="step2" {...fadeInUp} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  <input type="text" placeholder="Name" style={{ padding: '14px', borderRadius: '14px', border: '1px solid #eee' }} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label style={{ fontSize: '0.9rem', fontWeight: '700' }}>Mobile</label>
                  <input type="tel" placeholder="Number" style={{ padding: '14px', borderRadius: '14px', border: '1px solid #eee' }} />
                </div>
              </div>
          ...
          {step === 3 && (
            <motion.div key="step3" {...fadeInUp} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                <input type="text" placeholder="State" style={{ padding: '14px', borderRadius: '14px', border: '1px solid #eee' }} />
                <input type="text" placeholder="District" style={{ padding: '14px', borderRadius: '14px', border: '1px solid #eee' }} />
              </div>
              <textarea placeholder="Full Address" rows={3} style={{ padding: '14px', borderRadius: '14px', border: '1px solid #eee' }}></textarea>
              <div style={{ display: 'flex', gap: '15px', marginTop: '20px' }}>
                <button type="button" onClick={prevStep} className="btn-secondary" style={{ flex: 1, justifyContent: 'center' }}>Back</button>
          ...
          {step === 4 && (
            <motion.div key="step4" {...fadeInUp} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ display: 'flex', gap: '15px', marginTop: '20px' }}>
                <button type="button" onClick={prevStep} className="btn-secondary" style={{ flex: 1, justifyContent: 'center' }}>Back</button>
                <button type="submit" className="btn-primary" style={{ flex: 1, justifyContent: 'center' }}>Join SakhiHub <Sparkles size={18} /></button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
