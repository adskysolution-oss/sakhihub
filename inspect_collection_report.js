...
async function test() {
  ...
  const userHierarchy = await User.aggregate([
    { $match: { _id: targetUser ? targetUser._id : null } },
    {
      $graphLookup: {
        from: 'users',
        startWith: '$_id',
  ...
  descendants.forEach(d => {
    console.log(`  - Descendant: Name=${d.fullName}, Role=${d.role}, ID=${d._id.toString()}`);
  });

  const directAndIndirectUserIds = targetUser ? [targetUser._id, ...descendants.map(d => d._id)] : [];
  const vendorCodes = targetUser ? [targetUser, ...descendants].filter(u => u.role === 'vendor' && u.vendorCode).map(u => u.vendorCode) : [];
  const subVendorCodes = targetUser ? [targetUser, ...descendants].filter(u => u.role === 'sub_vendor' && u.subVendorCode).map(u => u.subVendorCode) : [];

  console.log(`directAndIndirectUserIds:`, directAndIndirectUserIds.map(id => id.toString()));
